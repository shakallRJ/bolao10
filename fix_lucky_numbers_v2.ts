...
async function runMigration() {
  ...
  console.log('Running migration to create lucky_numbers table via execute_sql/sql_query...');
  try {
    const { data, error } = await supabase.rpc('execute_sql', { sql_query: sql });
    
    if (error) {
      console.log('Migration RPC Error (execute_sql/sql_query):', JSON.stringify(error, null, 2));
    } else {
